...
import {
    IVaultBridgeTokenInitializer, NativeConverterInfo, InitDataStruct
} from "./etc/IVaultBridgeTokenInitializer.sol";
// Other.
...
/// @dev A base contract used to create vault bridged tokens.
    ...
    IVersioned
{
    ...
    function __VaultBridgeToken_init(InitDataStruct calldata data_, address initializer_) internal virtual initializer {
        require(initializer_ != address(0));
        (bool success, bytes memory result) =
            initializer_.delegatecall(abi.encodeCall(IVaultBridgeTokenInitializer.__VaultBackedTokenInit_init, (data_)));
        if (!success) {
    ...
    /// @notice The number of decimals of the vault bridged token.
    ...
    /// @notice Vault bridged tokens have an internal reserve of the underlying token from which withdrawals are served first.
    ...
    /// @dev vbToken is bridged by the underlying token 1:1.
    ...
    /// @dev The message tells vbToken how much Custom Token must be bridged by vbToken, which is minted and bridged to address zero on the respective Layer Y. This action provides liquidity when bridging Custom Token to from Layer Ys to Layer X and increments the pessimistic proof.
    ...
    function _completeMigration(uint32 originNetwork, uint256 shares, uint256 assets) internal {
        ...
        // A discrepancy is possible due to transfer fees of the underlying token. To offset the discrepancy, we mint more vbToken, bridged by assets from the dedicated migration fees fund.
    ...
    function setTransferFeeUtil(address transferFeeUtil_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        VaultBridgeTokenStorage storage $ = _getVaultBridgeTokenStorage();
    ...
    /// @notice This function can be called by the owner only.
    function setNativeConverters(NativeConverterInfo[] calldata nativeConverters_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
        nonReentrant
    {
