...
methods {
    function underlyingTokenContract.balanceOf(address) external returns (uint256) envfree;
    function underlyingTokenContract.totalSupply() external returns (uint256) envfree;
    function underlyingTokenContract.transfer(address,uint256) external returns (bool);
}
...
hook Sload uint256 _balance underlyingTokenContract._balanceOf[KEY address account] {
    require ghost_balances[account] == _balance;
}

hook Sstore underlyingTokenContract._balanceOf[KEY address account] uint256 _balance (uint256 _balance_old) {
...
invariant sumOfBalancesEqualsTotalSupply()
    sumOfBalances[2^160] == to_mathint(underlyingTokenContract.totalSupply())
    filtered { f -> !excludedMethod(f) }
    {
        preserved {
            requireLinking();
...
rule twoBalancesCannotExceedTotalSupply(address accountA, address accountB) {
    ...
    uint256 balanceA = underlyingTokenContract.balanceOf(accountA);
    uint256 balanceB = underlyingTokenContract.balanceOf(accountB);

    assert accountA != accountB =>
        balanceA + balanceB <= to_mathint(underlyingTokenContract.totalSupply());
...
rule threeBalancesCannotExceedTotalSupply(address accountA, address accountB, address accountC) {
    ...
    uint256 balanceA = underlyingTokenContract.balanceOf(accountA);
    uint256 balanceB = underlyingTokenContract.balanceOf(accountB);
    uint256 balanceC = underlyingTokenContract.balanceOf(accountC);

    assert accountA != accountB && accountA != accountC && accountB != accountC =>
        balanceA + balanceB + balanceC <= to_mathint(underlyingTokenContract.totalSupply());
    satisfy(accountA != accountB && balanceA + balanceB + balanceC > to_mathint(underlyingTokenContract.totalSupply()));
}
