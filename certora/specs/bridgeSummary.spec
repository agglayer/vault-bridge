methods {
    function _.bridgeAsset(uint32,address,uint256,address,bool,bytes) external => NONDET; // this is UNSAFE summary.
    
    function _.claimAsset(
        bytes32[32], bytes32[32], uint256, bytes32, bytes32, 
        uint32, address, uint32, address, uint256, bytes) external => NONDET; // this is UNSAFE summary.
