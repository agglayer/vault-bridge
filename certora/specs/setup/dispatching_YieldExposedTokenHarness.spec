using IERC20PlusMock as IERC20PlusMock;
using Vault as Vault;

methods {
    function YieldExposedTokenHarness.token() external
        returns (address) => CVL_token();
    function YieldExposedTokenHarness.yieldGeneratingVault() external
        returns (address) => CVL_vault();

    function YieldExposedTokenHarness.balanceOf(address) external
        returns (uint256) => NONDET;
    function YieldExposedTokenHarness.totalSupply() external
        returns (uint256) => NONDET;
    function YieldExposedTokenHarness.transfer(address,uint256) external
        returns (bool) => NONDET;
    function YieldExposedTokenHarness.allowance(address,address) external
        returns (uint256) => NONDET;
    function YieldExposedTokenHarness.approve(address,uint256) external
        returns (bool) => NONDET;
    function YieldExposedTokenHarness.transferFrom(address,address,uint256) external
        returns (bool) => NONDET;

    function _.proxiableUUID() external => DISPATCHER(true);
    function _.balanceOf(address) external => DISPATCHER(true);
    function _.previewWithdraw(uint256) external => DISPATCHER(true);
    function _.maxRedeem(address) external => DISPATCHER(true);
    function _.totalAssets() external => DISPATCHER(true);
    function _.withdraw(uint256,address,address) external => DISPATCHER(true);
    function _.redeem(uint256,address,address) external => DISPATCHER(true);
    function _.accrueYield() external => DISPATCHER(true);
    function _.maxWithdraw(address) external => DISPATCHER(true);
    function _.convertToAssets(uint256) external => DISPATCHER(true);
    function _.transfer(address,uint256) external => DISPATCHER(true);
    function _.transferFrom(address,address,uint256) external => DISPATCHER(true);
    function _.maxDeposit(address) external => DISPATCHER(true);
    function _.deposit(uint256,address) external => DISPATCHER(true);
    function _.convertToShares(uint256) external => DISPATCHER(true);
    function _.approve(address,uint256) external => DISPATCHER(true);
}

function CVL_token() returns address {
    return IERC20PlusMock;
}
function CVL_vault() returns address {
    return Vault;
}
